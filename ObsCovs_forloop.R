...
for(i in 1:length(sites)) {
  ...
  if(curdata$Survey==1){ occdata$Obs1[i] <- curdata$Observer }
  if(curdata$Survey==2){ occdata$Obs2[i] <- curdata$Observer }
}

  curdata <- phibs[which(phibs$Site.Code==site & phibs$Year==2017),] ##get data from current site in 2017

  if(curdata$Survey==1){ occdata$Obs3[i] <- curdata$Observer }
  if(curdata$Survey==2){ occdata$Obs4[i] <- curdata$Observer }
}
