#########################################################################
#ASTR data
#########################################################################

#setwd("C:\\Users\\db1876\\Google Drive\\alyssa\\20170104")

#Importing and formatting data: csvToUMF
install.packages("unmarked")
library(unmarked)

data <- read.csv("occdata_ASTR_Robust.csv")#load your data in
SiteCovs<-read.csv("SiteCovs.csv")
data$y.1<-as.integer(data$y.1)
data$y.2<-as.integer(data$y.2)
data$y.3<-as.integer(data$y.3)
data$y.4<-as.integer(data$y.4)
data$sites<-as.integer(data$sites)

str(data)
#GW_torrent<-subset(torrent,Stream=="1")
y <- data[,2:5] #makes columns 2-3 (the occupancy data) into a variable called 'y'

siteCovs <- SiteCovs[,c("Watershed","Tier","Sediment","Coarse","WW","Over","Depth","Gradient")]
siteCovs$Watershed <- as.factor(siteCovs$Watershed)
siteCovs$Tier <- as.factor(siteCovs$Tier)

#obsCovs <- list(date=wt[,c("date.1", "date.2", "date.3")],
# ivel=wt[,c("ivel.1", "ivel.2", "ivel.3")])
#obsCovs1<-list(observer=GW_torrent[,c("Observer.1","Observer.2")],
#              Latency=GW_torrent[,c("Latency.1","Latency.2")])

ASTR <- unmarkedFrameOccu(y=y,siteCovs = siteCovs)
summary(ASTR)


#Use long = TRUE if sites dont have the same numbers of observations


#To help stabilize the numerical optimization algorithm, its recommended standardizing the covariates
#obsCovs(torrent) <- scale(obsCovs(torrent))
siteCovs(ASTR)[,3:8] <- scale(siteCovs(ASTR)[,3:8])


#Fitting models

fm1 <- occu(~1 ~1, ASTR)
fm1
backTransform(fm1, 'state')#prob of occupancy
backTransform(fm1,'det')   #prob of detection
#Fail to reject the null & conclude the model fit is NOT adequate

fm2<-occu(~1 ~Watershed, ASTR)
fm2
backTransform(linearComb(fm2,coefficients=c(1,0),type='state'))
backTransform(fm2,type='det')

fm3<-occu(~1 ~Tier, ASTR)
fm3
backTransform(linearComb(fm3,coefficients=c(1,0),type='state'))
backTransform(fm3,type='det')
#Fail to reject the null & conclude the model fit is NOT adequate

fm4<-occu(~1 ~Sediment, ASTR)
fm4
backTransform(linearComb(fm4,coefficients=c(1,0),type='state'))
backTransform(fm4,type='det')

##easy way to make a bad graph:
plot(siteCovs(ASTR)$Sediment,predict(fm4,type='state')[,1])

fm5<-occu(~1 ~Watershed + Sediment, ASTR)
fm5
backTransform(linearComb(fm5,coefficients=c(1,0,0),type='state'))
backTransform(fm5,type='det')
#Fail to reject the null & conclude the model fit is NOT adequate

fm6<-occu(~1 ~WW, ASTR)
fm6
backTransform(linearComb(fm6,coefficients=c(1,0),type='state'))
backTransform(fm6,type='det')

fm7<-occu(~1 ~Gradient, ASTR)
fm7
backTransform(linearComb(fm6,coefficients=c(1,0),type='state'))
backTransform(fm7,type='det')
#Fail to reject the null & conclude the model fit is NOT adequate

fm8<-occu(~1 ~Gradient+WW+Sediment+Depth+Over+Coarse+Watershed*Tier, ASTR)
fm8
backTransform(linearComb(fm8,coefficients=c(1,0,0,0,0,0,0,1,1),type='state'))
backTransform(fm8,type='det')

fm9<-occu(~1 ~Gradient+WW+Sediment+Depth+Over+Coarse+Watershed, ASTR)
fm9
backTransform(linearComb(fm9,coefficients=c(1,0,0,0,0,0,0,0),type='state'))
backTransform(fm9,type='det')

fm10<-occu(~1 ~Gradient+Coarse+Watershed, ASTR)
fm10
backTransform(linearComb(fm10,coefficients=c(1,0,0,0),type='state'))
backTransform(fm10,type='det')

fm11<-occu(~1 ~WW+Sediment+Depth+Over+Coarse+Watershed+Tier, ASTR)
fm11
backTransform(linearComb(fm11,coefficients=c(1,0,0,0,0,0,0,0),type='state'))
backTransform(fm11,type='det')

fm12<-occu(~1 ~Over, ASTR)
fm12
backTransform(linearComb(fm12,coefficients=c(1,0),type='state'))
backTransform(fm11,type='det')
#Fail to reject the null & conclude the model fit is NOT adequate

fm13<-occu(~1 ~Coarse, ASTR)
fm13
backTransform(linearComb(fm13,coefficients=c(1,0),type='state'))
backTransform(fm11,type='det')

fm14<-occu(~1 ~Depth, ASTR)
fm14
backTransform(linearComb(fm14,coefficients=c(1,0),type='state'))
backTransform(fm11,type='det')

#2
#Model Selection and Model Fit
fms<-fitList(fm1,fm2,fm3,fm4,fm5,fm6,fm7,fm8,fm9,fm10,fm11,fm12,fm13,fm14)
modSel(fms)
#model weights = probabilit ythat model k is the "best" model in the candidate set
#              = the weight of evidence in favor of model k being the actual Kullback-Leibler best model for the situation at hand

####################################################################
#Goodness of Fit# Parametric Bootstrap Method
####################################################################
chisq <- function(fm) {
  umf <- getData(fm)
  y <- getY(umf)
  y[y>1] <- 1
  sr <- fm@sitesRemoved
  if(length(sr)>0)
    y <- y[-sr,,drop=FALSE]
  fv <- fitted(fm, na.rm=TRUE)
  y[is.na(fv)] <- NA
  sum((y-fv)^2/(fv*(1-fv)), na.rm=TRUE)
}
(pb <- parboot(fm1, statistic=chisq, nsim=100, parallel=FALSE))
#Fail to reject the null & conclude the model fit is NOT adequate
(pb <- parboot(fm2, statistic=chisq, nsim=100, parallel=FALSE))
(pb <- parboot(fm3, statistic=chisq, nsim=100, parallel=FALSE))
#Fail to reject the null & conclude the model fit is NOT adequate
(pb <- parboot(fm4, statistic=chisq, nsim=100, parallel=FALSE))
(pb <- parboot(fm5, statistic=chisq, nsim=100, parallel=FALSE))
#Fail to reject the null & conclude the model fit is NOT adequate
(pb <- parboot(fm6, statistic=chisq, nsim=100, parallel=FALSE))
(pb <- parboot(fm7, statistic=chisq, nsim=100, parallel=FALSE))
#Fail to reject the null & conclude the model fit is NOT adequate
(pb <- parboot(fm8, statistic=chisq, nsim=1000, parallel=FALSE))
(pb <- parboot(fm9, statistic=chisq, nsim=100, parallel=FALSE))
(pb <- parboot(fm10, statistic=chisq, nsim=100, parallel=FALSE))
(pb <- parboot(fm11, statistic=chisq, nsim=100, parallel=FALSE))
(pb <- parboot(fm12, statistic=chisq, nsim=100, parallel=FALSE))
#Fail to reject the null & conclude the model fit is NOT adequate
(pb <- parboot(fm13, statistic=chisq, nsim=100, parallel=FALSE))
(pb <- parboot(fm14, statistic=chisq, nsim=100, parallel=FALSE))

#################################################################
#CI_Empirical Bayes Method
#################################################################

re <- ranef(fm11)
EBUP <- bup(re, stat="mode")
CI <- confint(re, level=0.9)
rbind(PAO = c(Estimate = sum(EBUP), colSums(CI)) / 130)
#if p>0.05, then we fail to reject the null hypoth and conclude that 
#the model fit is adequate
#POA = proportion of sites occupied
#################################################################

amphib<-read.csv("amphib.csv")
quantile(na.omit(amphib$TL))
str(amphib$SVL)
amphib$TL<-as.numeric(amphib$TL)
amphib$SVL<-as.numeric(amphib$TL)
