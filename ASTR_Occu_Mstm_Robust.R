...
data <- read.csv("occdata_ASTR_MNSTM.csv")#load your data in
SiteCovs<-read.csv("SiteCovs_MSTM.csv")
...
fm8<-occu(~Gradient+WW+Sediment+Depth+Over+Coarse+Watershed ~1, ASTR)
fm8
backTransform(linearComb(fm8,coefficients=c(1,0,0,0,0,0,0,0),type='det'))
backTransform(fm8,type='state')

fm10<-occu(~Gradient+Coarse+Watershed ~ 1, ASTR)
fm10
backTransform(linearComb(fm10,coefficients=c(1,0,0,0),type='det'))
backTransform(fm10,type='state')

fm11<-occu(~WW+Sediment+Depth+Over+Coarse+Watershed ~1, ASTR)
fm11
backTransform(linearComb(fm11,coefficients=c(1,0,0,0,0,0,0),type='det'))
backTransform(fm11,type='state')



#Model Selection and Model Fit
fms<-fitList('psi(.)p(.)'=fm1,'psi(Watershed)p(.)'= fm2,
             'psi(Sediment)p(.)'= fm4, 'psi(Watershed+Sediment)p(.)'= fm5,'psi(WW)p(.)'= fm6,
             'psi(Gradient)p(.)'= fm7,'psi(Gradient+WW+Sediment+Depth+Over+Coarse+Watershed)p(.)'= fm8,
             'psi(Gadient+Coarse+Watershed)p(.)'=fm10,
             'psi(WW+Sediment+Depth+Over+Coarse+Watershed)p(.)'= fm11)
modSel(fms)
